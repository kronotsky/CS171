...
void raster(Object &o, Face &f, vector<Light> lights, Camera &c, Grid &gr, Buffer &buf) {
    Eigen::Vector3d va, vb, vc, cross, curr;
	...
	for (y = ymin; y <= ymax; y++) {
		...
		(0 <= gamma) && (gamma <= 1)) {
		curr = alpha * va + beta * vb + gamma * vc;
		
		if ((-1 <= curr(0)) && (curr(0) <= 1) && \
		    (-1 <= curr(1)) && (curr(1) <= 1) && \
		    (-1 <= curr(2)) && (curr(2) <= 1) && \
		    (curr(2) <= buf.get(x,y))) {
